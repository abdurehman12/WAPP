...
class CartController extends ChangeNotifier {
  ...
  Future<ApiResponse> updateCartProductQuantity(int? key, int quantity, BuildContext context, bool increment, int index) async {
    ...
    ApiResponse apiResponse = await cartServiceInterface!.updateQuantity(key, quantity);
    if (apiResponse.response != null && apiResponse.response!.statusCode == 200) {
  ...
  //     List<ChoiceOptions> choices, List<int>? variationIndexes, {int buyNow = 0}) async {
  ...
  Future<ApiResponse> addToCartAPI(CartModelBody cart, BuildContext context,
    ...
    _addToCartLoading = false;
    if (apiResponse.response != null && apiResponse.response!.statusCode == 200) {
      // Use context passed to the method instead of Get.context
  ...
  // Add this to your CartController if not already present
  Future<double> getTotalWeight() async {
    double totalWeight = 0.0;
    for (CartModel cart in cartList) {
      totalWeight += (cart.weight ?? 0.5) * (cart.quantity ?? 1);
    }
    return totalWeight;
  }
}