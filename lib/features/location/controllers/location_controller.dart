...
void updateMapPosition(CameraPosition? position, bool fromAddress, String? address, BuildContext context) async {
  if (_updateAddAddressData) {
    ...
    notifyListeners();
    try {
      if (position != null) { // Ensure position is not null
        if (fromAddress) {
          _position = Position(
        ...
        if (_changeAddress) {
          ...
          if (fromAddress) {
            _address = Placemark(name: addressFromGeocode);
          } else {
            _pickAddress = Placemark(name: addressFromGeocode);
          }

          _locationController.text = address ?? placeMarkToAddress(fromAddress ? _address : _pickAddress!);
        } else {
          _changeAddress = true;
        }
      } else {
        if (kDebugMode) {
          print("Camera position is null.");
        }
      }
    } catch (e) {
      if (kDebugMode) {
